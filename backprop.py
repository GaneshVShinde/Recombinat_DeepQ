...
class NN:
    ...
    def classificationCorrectness(self,patterns):
      count=0.0
      for input,target in patterns:
        error,output=self.testOne(input,target)
        if len(target)>1:
          ...
          if correct==predict:
            count+=1
        else:    # fix this to check the target array to see what the possible targets are!
          if target[0]<0 and output<0: count+=1
          elif target[0]>0 and output>0: count+=1
